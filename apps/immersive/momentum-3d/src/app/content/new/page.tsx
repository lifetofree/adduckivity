...
export default function NewPostPage() {
  ...
  const runAI = async (section: AiSection) => {
    ...
    try {
      switch (section) {
        case 'titles': {
          const res = await suggestTitles(content, title)
          setAiResults(p => ({ ...p, titles: res }))
          break
        }
        case 'excerpt': {
          const res = await autoExcerpt(title, content)
          setAiResults(p => ({ ...p, excerpt: res }))
          break
        }
        case 'outline': {
          const res = await buildOutline(title, content)
          setAiResults(p => ({ ...p, outline: res }))
          break
        }
        case 'seo': {
          const res = await seoTips(title, excerpt, content, tags.split(',').map(t => t.trim()).filter(Boolean))
          setAiResults(p => ({ ...p, seo: res }))
          break
        }
        case 'tags': {
          const res = await suggestTags(title, content)
          setAiResults(p => ({ ...p, tags: res }))
          break
        }
      }
    } catch {
      setToast({ type: 'err', msg: 'AI request failed' })
    } finally {
      setAiLoading(null)
    }
  }
    ...
    try {
      const res = await fetch('/api/posts/create', {
        ...
        body: JSON.stringify({
          ...
          category, featuredImage, scene, mood,
        }),
      })
      if (!res.ok) throw new Error((await res.json()).error || 'Failed')
      const post = await res.json()
      setToast({ type: 'ok', msg: 'Published!' })
      setTimeout(() => { window.location.href = `/content/${post.slug}` }, 1000)
    } catch (err: any) {
    ...
    <div className="h-screen flex flex-col bg-[#FBFBFA] text-[#141414] overflow-hidden">
      ...
      <header className="shrink-0 h-12 flex items-center justify-between px-5 border-b border-[#E8E8E6] bg-white z-30">
        <div className="flex items-center gap-3 text-sm">
          <Link href="/content" className="text-gray-400 hover:text-gray-700 transition-colors">
            ← Posts
          </Link>
        ...
        <div className="flex items-center gap-4 text-xs text-gray-400">
          ...
          <button
            ...
            className="ml-2 h-8 px-5 rounded-md bg-[#141414] text-white text-xs font-medium hover:bg-[#2a2a2a] disabled:opacity-40 transition-colors"
          >
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <SideSection label="Post Metadata" open={open.meta} onToggle={() => toggle('meta')}>
            ...
            <Field label="Excerpt">
              <textarea
                ...
                rows={3}
                className="base-input resize-none text-xs leading-relaxed"
              />
              <ProgressBar value={excerptLen} max={160} optimal={[120, 160]} />
              <span className="text-[10px] text-gray-400 mt-0.5 block">{excerptLen}/160</span>
            </Field>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-10 pt-8 pb-3 border-b border-[#E8E8E6]">
            <input
              ...
              required
              className="w-full bg-transparent text-[2rem] font-bold leading-tight placeholder-gray-200 focus:outline-none"
            />
            <div className="flex items-center justify-between mt-2">
              <span className={`text-[11px] ${titleLen > 60 ? 'text-red-500' : 'text-gray-400'}`}>
          ...
          <div className="flex-1 overflow-y-auto px-10 py-6">
            <textarea
              ...
              required
              className="w-full h-full min-h-[500px] bg-transparent text-base leading-8 placeholder-gray-200 focus:outline-none resize-none font-[var(--font-inter)]"
            />
          </div>
        </main>
        ...
        <aside className="w-72 shrink-0 border-l border-[#E8E8E6] bg-white overflow-y-auto">
          ...
          >
            {aiResults.titles?.map((t, i) => (
              <div key={i} className="flex items-start gap-2 group py-1">
                ...
                <button
                  onClick={() => setTitle(t)}
                  className="flex-1 text-left text-xs text-gray-700 hover:text-violet-700 leading-relaxed transition-colors"
                >
                  {t}
                </button>
                <button
                ...
                >
                  {copied === `title-${i}` ? <Check size={12} className="text-green-500" /> : <Copy size={12} className="text-gray-300" />}
                </button>
              </div>
            ))}
          </AISection>
          ...
          >
            {aiResults.excerpt && (
              <div className="space-y-2">
                <p className="text-xs text-gray-600 leading-relaxed">{aiResults.excerpt}</p>
                <button
                  onClick={() => setExcerpt(aiResults.excerpt!)}
                  className="text-[11px] px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors"
                >
                  Apply
                </button>
              </div>
          ...
          >
            {aiResults.outline && (
              <div className="max-h-52 overflow-y-auto space-y-0.5">
                {aiResults.outline.map((line, i) => (
                  <p key={i} className="text-xs text-gray-600 font-mono leading-relaxed whitespace-pre">{line}</p>
                ))}
              </div>
            )}
          </AISection>
          ...
          <AISection
            ...
            color="amber"
          >
            {aiResults.seo?.map((tip, i) => (
              <p key={i} className="text-xs text-gray-600 leading-relaxed py-0.5 border-b border-gray-100 last:border-0">{tip}</p>
            ))}
          </AISection>
            ...
            {aiResults.tags && (
              <div className="space-y-2">
                <div className="flex flex-wrap gap-1.5">
                  {aiResults.tags.map((t, i) => (
                    <span key={i} className="px-2 py-0.5 bg-pink-50 text-pink-700 text-[11px] rounded-full border border-pink-200">
                      {t}
                    </span>
                  ))}
                </div>
                <button
                  onClick={() => setTags(aiResults.tags!.join(', '))}
                  className="text-[11px] px-3 py-1 bg-pink-600 text-white rounded hover:bg-pink-700 transition-colors"
                >
                  Apply Tags
                </button>
              </div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className={`fixed bottom-5 right-5 px-5 py-3 rounded-lg shadow-lg text-sm font-medium ${toast.type === 'ok' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}
          >
            {toast.msg}
          </motion.div>
        )}
...
function AISection({ label, sectionKey, open, onToggle, loading, onRun, color, children }: {
  label: string; sectionKey: string; open: boolean; onToggle: () => void
  loading: boolean; onRun: () => void; color: string; children?: React.ReactNode
}) {
  return (
    <div className="border-b border-[#E8E8E6]">
      <div className="flex items-center justify-between px-4 py-2.5">
        ...
        <button
          onClick={onRun}
          disabled={loading}
          className={`p-1.5 rounded-md transition-colors ${colorMap[color]}`}
...
function ProgressBar({ value, max, optimal, className = '' }: {
  ...
  const color = over ? '#ef4444' : inOptimal ? '#22c55e' : '#3b82f6'
  return (
