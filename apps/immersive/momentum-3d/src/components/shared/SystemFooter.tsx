...
export default function SystemFooter() {
  const { isFooterVisible, footerNode } = useSystem()
    ...
    <AnimatePresence>
      {isFooterVisible && (
        <motion.footer 
          ...
          exit={{ opacity: 0, y: 20 }}
          className="z-10 h-14 border-t border-white/5 bg-black/20 backdrop-blur-md flex items-center justify-between px-6"
        >
          {/* Left: kernel tag */}
