...
- **`IgnitionOverlay.tsx`:** Handles the phase-specific text prompts and timers. Renders on top of the 3D scene during ignition.
- **`useIgnitionScene.ts`:** A hook that updates `ProtocolScene` uniforms (lighting, colors) based on the current ignition phase.
...
interface IgnitionState {
  ...
  isActive: boolean;
  targetNodeId: string | null; // The node to transition to after completion
}
```
**Critical Logic:** The `targetNodeId` must be cleared via `stop()` immediately after the handoff to prevent navigation loops ("bounces").
...
4. **Handoff:** Upon completion (T-0), the `targetNodeId` is used to trigger a camera flight to the next node. The ignition state is then explicitly cleared.
5. **Flow:** User enters "Flow Mode" (Pilot view). 
6. **Completion:** When the final node's timer finishes (or is manually completed), the "Next Step" button is disabled and replaced by a global **Protocol Complete** state card.

---

## 5. Success Criteria
- [x] Implement a state machine that transitions through the 3 phases.
- [x] Integration with `ProtocolScene` to change colors/ambient lighting per phase.
- [x] Large, legible countdown timer overlay.
- [x] Successful "Flight" transition at the end of the 600 seconds.
- [x] Mobile-responsive UI for the timer/prompts.
